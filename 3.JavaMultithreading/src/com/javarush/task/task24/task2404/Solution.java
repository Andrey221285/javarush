...
public class Solution {
    ...
    public static class Rectangle{
        ...
        public HasHeight castToHasHeight() {
            class MyImplementationOfHasHeight implements HasHeight {
                @Override
                public double getHeight() {
                    double delta = point1.getY() - point2.getY();
                    return delta > 0 ? delta : -delta;
                }
            } return new MyImplementationOfHasHeight();
        }


        public HasWidth castToHasWidth() {
            class MyImplementationOfHasWidth implements HasWidth {
                @Override
                public double getWidth() {
                    double delta = point1.getX() - point2.getX();
                    return delta > 0 ? delta : -delta;
                }
            } return new MyImplementationOfHasWidth();
        }
